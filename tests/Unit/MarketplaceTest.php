...
class MarketplaceTest extends TestCase
{
    ...
    public function test_extract_product_id_from_id()
    {
        ...
        $urls = [
            ...
            "https://ar.alibaba.com/product-detail/1600795066971.html",
        ];
        foreach($urls as $url) {
            $extractData = $service->extractProductId($url);
            $this->assertNotNull($extractData, $url);
            $this->assertIsArray($extractData, $url);
            $this->assertTrue(isset($extractData["source"]), $url);
            $this->assertTrue(isset($extractData["id"]), $url);
        }
    }
}
